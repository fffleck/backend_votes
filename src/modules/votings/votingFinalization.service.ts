...
import { decrypt } from "../../providers/crypto"

type VoteAnswerSet = Record<string, string | string[]>
type AggregatedOption = { label: string; count: number }
type AggregatedStep = { title: string; type: string; options: AggregatedOption[] }
...
function getLogoBuffer() {
  ...
  cachedLogo = fsSync.existsSync(LOGO_PATH) ? fsSync.readFileSync(LOGO_PATH) : null
  return cachedLogo
}
...
function getPublicDownloadUrl(relativeUrl: string) {
  const baseUrl =
...
class PdfBuilder {
  ...
  private hasLogo = Boolean(getLogoBuffer())

  constructor(private title: string, private options: { branded?: boolean; downloadUrl?: string } = {}) {
    this.newPage()
  }
  ...
  private drawQrCode(value: string, x: number, y: number, size: number) {
    const qr = QRCode.create(value, { errorCorrectionLevel: "M" }) as any
  ...
  private ensureSpace(height: number) {
    ...
    if (this.y - height < bottomLimit) this.newPage()
  }

  ensureSectionSpace(height: number) {
    this.ensureSpace(height)
  }

  text(value: string, x = MARGIN, y = this.y, size = 11, bold = false, color = "0.08 0.08 0.08") {
    const font = bold ? "F2" : "F1"
    this.content += `BT /${font} ${size} Tf ${color} rg ${x} ${y} Td ${toPdfHex(value)} Tj ET\n`
  }

  line(value: string, size = 11, bold = false, gap = 17) {
    this.ensureSpace(gap + 4)
    this.text(value, MARGIN, this.y, size, bold)
    this.y -= gap
  }

  paragraph(value: string, size = 10, maxChars = 86) {
  ...
  optionBar(label: string, count: number, total: number, color: string) {
    this.ensureSpace(48)
    const pct = total > 0 ? count / total : 0
    const width = 305
    const filled = Math.max(width * pct, count > 0 ? 3 : 0)
  ...
  tableRow(values: string[], widths: number[], bold = false, backgroundColor?: string, textColor = "0.08 0.08 0.08") {
    ...
    values.forEach((_value, index) => {
      for (let i = 0; i < rowLines; i++) {
        if (lines[index][i]) this.text(lines[index][i], x, startY - 13 * i, 9, bold, textColor)
      }
      x += widths[index]
    })
...
function createPdf(pageContents: string[], logoBuffer: Buffer | null = null) {
...
export class VotingFinalizationService {
  async finalize(votingId: string) {
    ...
    await fs.writeFile(
      path.join(DOWNLOADS_DIR, resultFilename),
      this.buildResultPdf(voting.title, finalizedAt, aggregated, voting.votes.length, eligibleUsers.length, resultPdfDownloadUrl)
    )
    await fs.writeFile(
      path.join(DOWNLOADS_DIR, nonVotersFilename),
      this.buildNonVotersPdf(voting.title, finalizedAt, nonVoters, nonVotersPdfDownloadUrl)
    )
  ...
  private parseVotes(votes: Array<{ encryptedVote: string }>) {
    return votes.map(vote => {
      try {
        const decrypted = JSON.parse(decrypt(vote.encryptedVote))
        return decrypted.answers as VoteAnswerSet
      } catch {
        return null
      }
    }).filter(Boolean) as VoteAnswerSet[]
  }

  private aggregateResults(
    steps: Array<{ id: string; title: string; type: string; options: Array<{ id: string; label: string }> }>,
    answers: VoteAnswerSet[]
  ) {
  ...
  private buildResultPdf(
    votingTitle: string,
    finalizedAt: Date,
    steps: AggregatedStep[],
    totalVotes: number,
    eligibleUsersTotal: number,
    downloadUrl?: string
  ) {
  ...
  private buildNonVotersPdf(
    votingTitle: string,
    finalizedAt: Date,
    nonVoters: Array<{ name: string; email: string }>,
    downloadUrl?: string
  ) {
    ...
    nonVoters.forEach((user, index) => {
      pdf.tableRow(
        ...
        [235, 260],
        false,
        index % 2 === 0 ? "0.96 0.96 0.97" : "1 1 1"
      )
    })
